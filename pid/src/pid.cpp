...
PID<T>::PID(float p, float i, float d, int set_point)
    : Node("pid_controller"), _p(p), _i(i), _d(d), _set_point(set_point),
      current_feedback(0), current_error(0), last_error(0),
      integral_cumulation(0), max_integral_comulation(0),
      cycle_derivative(0), output_lower_bound(0), output_upper_bound(0),
      last_time(0), current_time(0), use_time(false),
      use_output_bound(false), use_max_i_cum(false)
{
}
...
T PID<T>::calculation()
{
    ...
    if (use_time) {
        ...
        T cycle_integral = (last_error + current_error / 2) * delta_time;
    ...
    T output = (current_error * _p) + (integral_cumulation * _i) + (cycle_derivative * _d);
    if (use_output_bound) {
...
template <class T>
T PID<T>::getDerivative()
{
    return cycle_derivative * _d;
}

template <class T>
T PID<T>::getIntegralCum()
{
    return integral_cumulation;
}

template <class T>
void PID<T>::setP(float p)
{
    _p = p;
}

template <class T>
void PID<T>::setI(float i)
{
    _i = i;
}

template <class T>
void PID<T>::setD(float d)
{
    _d = d;
}

template <class T>
void PID<T>::setPID(float p, float i, float d)
{
    _p = p;
    _i = i;
    _d = d;
...
template <class T>
void PID<T>::setTarget(int set_point)
{
    _set_point = set_point;
}

template <class T>
void PID<T>::setFeedback(int feedback)
{
    current_feedback = feedback;
}

template <class T>
void PID<T>::setTime(unsigned long time)
{
    current_time = time;
}

template <class T>
void PID<T>::setMaxIntegralCum(int max_integral_cum)
{
    max_integral_comulation = max_integral_cum;
}

template <class T>
void PID<T>::setOutputBound(int lower_bound, int upper_bound)
{
